...
<html>
	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<div class="alus box box-info collapsed-box" style="float:right">
				<div class="box-header">
				...
				<div class="box-body" style="display:none">
					<form action="rekap_hasil.php" method="post">
						<div class="form-group">
							<select class="form-control" name="tgl_ujian" required>
								<option value="">Tanggal ujian</option>
								<?php
									include 'koneksi.php';
								    $sql = "SELECT * FROM hasil GROUP BY tgl_ujian ";
								    foreach ($DB_con->query($sql) as $data) :
								?>
								<option><?php echo $data['tgl_ujian'] ?></option>
								<?php endforeach; ?>
							</select>
						</div>
						<div class="form-group">
							<select class="form-control" name="nm_ujian" required>
								<option value="">Nama ujian</option>
								<?php
									include 'koneksi.php';
								    $sql = "SELECT * FROM hasil GROUP BY nm_ujian ";
								    foreach ($DB_con->query($sql) as $data) :
								?>
								<option><?php echo $data['nm_ujian'] ?></option>
								<?php endforeach; ?>
							</select>
						</div>
						<div class="form-group">
							<select class="form-control" name="kelas" required>
								...
								<?php
									include 'koneksi.php';
								    $sql = "SELECT * FROM hasil GROUP BY kelas ";
								    foreach ($DB_con->query($sql) as $data) :
								?>
						...
						<div class="form-group">
							<select class="form-control" name="nm_kelas" required>
								...
								<?php
									include 'koneksi.php';
								    $sql = "SELECT * FROM hasil GROUP BY nm_kelas ";
								    foreach ($DB_con->query($sql) as $data) :
								?>
				...
				<div class="box-footer clearfix">
					<button type="submit" class="pull-right btn btn-default">Tampilkan
        ...
        <div class="col-xs-12">
          <div class="box" style="margin-top:-10px">
            <div class="box-header">
              <h3 class="box-title">Semua Data Hasil Ujian</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover" id="myTable">
						...
						$i=1; 
							include 'koneksi.php';
						    $sql = "SELECT * FROM hasil,guru WHERE guru.nip = '$_SESSION[nisn_nip]' AND hasil.nm_pelajaran = guru.nm_pelajaran ORDER BY nilai desc";
						    foreach ($DB_con->query($sql) as $data) :
						?>
						<tr>
						<td><?php echo $i++;?></td>
